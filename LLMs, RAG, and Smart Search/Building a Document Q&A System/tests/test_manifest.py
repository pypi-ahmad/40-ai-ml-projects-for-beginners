...
def test_manifest_diff_and_save(tmp_path: Path) -> None:
    ...
    first = manager.diff(docs, embedding_model="qwen", chunk_size=256, chunk_overlap=32)
    ...
    second = manager.diff(docs, embedding_model="qwen", chunk_size=256, chunk_overlap=32)
    assert second.compatible is True
    assert second.added_or_changed_doc_ids == []

    third = manager.diff(docs, embedding_model="qwen", chunk_size=512, chunk_overlap=32)
